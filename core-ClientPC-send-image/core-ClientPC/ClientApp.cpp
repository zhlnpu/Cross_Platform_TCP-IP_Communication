#pragma once
#include "ClientApp.h"
#include <..\..\..\opencv310\build\include\opencv2\core\utility.hpp>
...
int C_Image::byte_size()
{
	int length = BaseDatatype::byte_size() + sizeof(uchar) * 3 * image_width* image_height;
...
void C_Image::ToCharArray(char buf[], Mat image)
{
	// copy inputType to buf
	...
	memcpy(&buf[pos], image.data, image.cols*image.rows*image.channels()*sizeof(uchar));

}


C_Image::C_Image(char data[]) : BaseDatatype(data)
{

	int pos = BaseDatatype::byte_size();
	int	 	 len = sizeof(uchar) * 3 * image_width* image_height;


	//Mat image(image_height, image_width, CV_8UC3);


	//	memcpy_s(&image.data[0], len, &data[pos], len);
...
void  ThisClient::Test()
{
	...
	C_Image cKun(CLIENT_NAME::CN_PC_1, DATA_NAME::DN_IMAGE, DATA_TYPE::DT_POINT3);

	// cKun._whom = CLIENT_NAME::CT_PC_2;
	char sendbuf[100 + sizeof(uchar) * 3 * image_width* image_height];
...
// 	}
	...
	while (key != 27)
	{
		...
		if (image.data)
		{
			cKun.ToCharArray(sendbuf, image);
			m_Connection.Send(sendbuf, cKun.byte_size());
...
ThisClient::ThisClient() :m_serverAddr("localhost")
{
	...
	if (m_Connection.IsConnected())
	{
		...
		char buf[100 + sizeof(uchar) * 3 * image_width* image_height];
...
void*  ProcessInput(void *arg)
{
	...
	char recvbuf[100 + sizeof(uchar) * 3 * image_width* image_height];
	int recvbuflen = 100 + sizeof(uchar) * 3 * image_width* image_height;
		...
		if (iResult > 0) {
			...
			if (fromWhom.whom == CLIENT_NAME::CN_PC_1)
			{
				...
				*/
				...
				//kun
				if (data_name == DATA_NAME::DN_IMAGE)
				{
					//	C_Image cKun(recvbuf);
					...
					int	 	 len = sizeof(uchar) * 3 * image_width* image_height;
					memcpy_s(&image.data[0], len, &recvbuf[sizeof(uchar) * 3], len);
					...
					cout << "fps" << getTickFrequency() / (GetTickCount() - start) / 1000 << endl;
